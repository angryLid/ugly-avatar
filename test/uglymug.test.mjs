...
import { useUglyMug } from "../src/lib/vue.js";
...
test("useUglyMug reproduces a seed and derives svg/dataUrl", () => {
  const h = useUglyMug("demo");
...
test("useUglyMug setSeed and regenerate work", () => {
  const h = useUglyMug();
  h.setSeed("fixed");
...
test("useUglyMug watches a reactive seed source", async () => {
  const seed = ref("one");
  const h = useUglyMug(seed);
